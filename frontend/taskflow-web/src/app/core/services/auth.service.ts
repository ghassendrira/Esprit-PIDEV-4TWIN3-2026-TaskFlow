...
export class AuthService {
  ...
  signup(payload: { firstName: string; lastName: string; email: string; companyName: string; companyCategory?: string; recaptchaToken: string; }) {
    return this.api.post<any>('/auth/signup', payload);
  }
  signin(payload: { email: string; password: string; recaptchaToken: string }) {
  ...
  googleSignup(payload: { code?: string; idToken?: string; companyName?: string; companyCategory?: string }) {
  ...
  getPasswordResetRequests() {
    return this.api.get<any[]>('/auth/password-reset-requests', { headers: this.authHeaders() });
  }

  getBlockedAccounts() {
    return this.api.get<any[]>('/admin/blocked-accounts', { headers: this.authHeaders() });
  }

  approvePasswordReset(requestId: string) {
    return this.api.post<any>(`/auth/password-reset-requests/${requestId}/approve`, {}, { headers: this.authHeaders() });
  }

  rejectPasswordReset(requestId: string, reason: string) {
    return this.api.post<any>(`/auth/password-reset-requests/${requestId}/reject`, { reason }, { headers: this.authHeaders() });
  }

  unblockAccount(userId: string) {
    return this.api.post<any>(`/admin/unblock/${userId}`, {}, { headers: this.authHeaders() });
  }
