    ...
    <div class="min-h-[calc(100vh-3rem)] p-6" style="background: var(--tf-surface); color: var(--tf-on-surface);">
      <div class="max-w-xl mx-auto">
        <div class="rounded-2xl p-6 border shadow-sm" style="background: var(--tf-card); border-color: var(--tf-border);">
          <h2 class="text-xl font-bold mb-2">{{ 'security.title' | t }}</h2>
          <p class="text-sm muted mb-4">{{ 'security.description' | t }}</p>

          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
            <div>
              <label class="block text-sm muted mb-1">{{ 'security.question' | t }}</label>
              <select
            ...
            <div>
              <label class="block text-sm muted mb-1">{{ 'security.answer' | t }}</label>
              <div class="relative">
                <input
            ...
            >
              <span *ngIf="!isSubmitting">{{ 'security.save' | t }}</span>
              <span *ngIf="isSubmitting">{{ 'security.saving' | t }}</span>
            </button>
          </form>
        </div>
      </div>
  ...
  onSubmit() {
    ...
    this.auth.setSecurityQuestions(payload).subscribe({
      next: (res: any) => {
        ...
        this.successMessage = this.language.translate('security.saved');
      },
      error: (err: any) => {
        this.isSubmitting = false;
        this.errorMessage = err?.error?.message ?? err?.message ?? this.language.translate('security.error');
      },
    });
  }
}
