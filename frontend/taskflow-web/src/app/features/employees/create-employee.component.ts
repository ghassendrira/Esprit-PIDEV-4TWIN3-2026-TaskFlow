    ...
    <div class="min-h-[calc(100vh-3rem)] p-6 bg-[var(--tf-surface)] text-[var(--tf-on-surface)]">
      <div class="max-w-xl mx-auto">
        <div class="bg-[var(--tf-card)] border border-[var(--tf-border)] rounded-2xl p-6 shadow-[var(--tf-shadow)]">
          <h2 class="text-xl font-bold mb-2">{{ 'employee.create-title' | t }}</h2>
          <p class="text-sm text-[var(--tf-muted)] mb-4">
            {{ 'employee.create-desc' | t }}
          </p>

          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
            <div>
              <label class="block text-sm text-[var(--tf-muted)] mb-1">{{ 'employee.email' | t }}</label>
              <input
                type="email"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-sm text-[var(--tf-muted)] mb-1">{{ 'employee.first-name' | t }}</label>
                <input
                  type="text"
              ...
              <div>
                <label class="block text-sm text-[var(--tf-muted)] mb-1">{{ 'employee.last-name' | t }}</label>
                <input
                  type="text"
            ...
            <div>
              <label class="block text-sm text-[var(--tf-muted)] mb-1">{{ 'employee.role' | t }}</label>
              <select
            ...
            >
              <span *ngIf="!isSubmitting">{{ 'employee.create' | t }}</span>
              <span *ngIf="isSubmitting">{{ 'employee.creating' | t }}</span>
            </button>
          </form>
        </div>
      </div>
    ...
    this.auth.createEmployee(payload).subscribe({
      next: (res: any) => {
        this.isSubmitting = false;
          this.successMessage =
          res?.message ?? this.language.translate('employee.success');
      },
      error: (err: any) => {
        this.isSubmitting = false;
        this.errorMessage =
          err?.error?.message ?? err?.message ?? this.language.translate('employee.error');
      },
    });
  }
}
