  ...
  template: `
    <div class="min-h-screen bg-[var(--tf-surface)] text-[var(--tf-on-surface)] font-['Syne'] selection:bg-primary/20 p-8 relative overflow-hidden">
      ...
      <div class="flex items-end justify-between mb-10">
        <div>
          <h1 class="text-5xl font-black mb-2 tracking-tighter uppercase leading-none">{{ 'employees.title' | t }}</h1>
          <p class="text-[var(--tf-muted)] font-medium tracking-tight">{{ 'employees.subtitle' | t }}</p>
        </div>
        <button 
      ...
      <div class="grid grid-cols-4 gap-6 mb-12">
        <div class="bg-[var(--tf-card)] p-6 rounded-2xl border border-[var(--tf-border)] group hover:border-primary/30 transition-all">
          <div class="text-[var(--tf-muted)] text-[10px] font-black uppercase tracking-[0.2em] mb-4">{{ 'employees.total' | t }}</div>
        ...
        <div class="bg-[var(--tf-card)] p-6 rounded-2xl border-t-2 border-t-[var(--tf-primary)] border-x border-x-[var(--tf-border)] border-b border-b-[var(--tf-border)]">
          <div class="text-[var(--tf-primary)] text-[10px] font-black uppercase tracking-[0.2em] mb-4">{{ 'employees.active' | t }}</div>
          <div class="text-4xl font-black leading-none">{{ stats().active }}</div>
        </div>
        <div class="bg-[var(--tf-card)] p-6 rounded-2xl border border-[var(--tf-border)] group hover:border-primary/30 transition-all">
          <div class="text-[var(--tf-muted)] text-[10px] font-black uppercase tracking-[0.2em] mb-4">{{ 'employees.roles' | t }}</div>
          <div class="text-4xl font-black leading-none">{{ stats().roles }}</div>
        </div>
        <div class="bg-[var(--tf-card)] p-6 rounded-2xl border border-[var(--tf-border)] group hover:border-primary/30 transition-all">
          <div class="text-[var(--tf-muted)] text-[10px] font-black uppercase tracking-[0.2em] mb-4">{{ 'employees.month' | t }}</div>
      ...
      <div class="flex flex-col gap-6 mb-8">
        <div class="relative group">
          <input 
            type="text" 
            [(ngModel)]="searchQuery"
            [placeholder]="'employees.search' | t"
      ...
      <div class="bg-[var(--tf-card)] rounded-[2rem] border border-[var(--tf-border)] overflow-hidden mb-8">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-[var(--tf-border)]">
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'employees.employee' | t }}</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'common.email' | t }}</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'employees.role' | t }}</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'employees.status' | t }}</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'employees.added' | t }}</th>
              <th class="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--tf-muted)]">{{ 'employees.actions' | t }}</th>
            </tr>
          </thead>
          <tbody>
            <tr 
              ...
              <td class="px-8 py-5">
                <div class="flex items-center gap-2.5">
                  ...
                  <span class="text-xs font-bold tracking-tight" [class.text-[var(--tf-on-surface)]]="emp.isActive" [class.text-[var(--tf-muted)]]="!emp.isActive">
                    {{ emp.isActive ? ('employees.active-label' | t) : ('employees.inactive-label' | t) }}
                  </span>
                </div>
              </td>
              <td class="px-8 py-5">
        ...
        <div *ngIf="filteredEmployees().length === 0" class="py-24 text-center">
          ...
          <div class="text-[var(--tf-muted)] font-black uppercase tracking-widest text-xs">{{ 'employees.no-employee' | t }}</div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between px-4">
        <div class="text-[10px] font-black uppercase tracking-widest text-[var(--tf-muted)]">
          {{ 'employees.showing' | t:{ current: filteredEmployees().length, total: employees().length } }}
        </div>
...
export class EmployeesListComponent implements OnInit {
  ...
  handleDialogConfirm() {
    ...
    const emp = this.pendingDeleteEmployee();
    if (!emp) {
      this.closeDialog();
      return;
    }
