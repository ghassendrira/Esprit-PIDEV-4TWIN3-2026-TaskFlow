...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, FormsModule, NgIf, NgFor, TranslatePipe],
  template: `
    <div class="flex min-h-[600px] overflow-hidden rounded-2xl border"
      ...
      <div class="w-[240px] border-r flex flex-col p-4 bg-[var(--tf-surface-2)]"
        ...
        <h1 class="text-lg font-semibold mb-6 px-2">{{ 'settings.title' | t }}</h1>
        ...
        <button (click)="showTenantModal = true" 
          ...
          style="border-color: var(--tf-border);">
          + {{ 'settings.new-company' | t }}
        </button>

        <!-- Navigation Section: Configuration -->
        <div class="space-y-1 mb-8">
          <p class="px-2 text-[10px] font-bold uppercase tracking-widest muted mb-2">{{ 'settings.configuration' | t }}</p>
          <button (click)="tab = 'company'" 
            ...
            <span>🏢</span> {{ 'settings.company-profile' | t }}
          </button>
          <button (click)="tab = 'security'" 
            ...
            <span>🛡️</span> {{ 'settings.security' | t }}
          </button>
          <button class="w-full px-3 py-2 rounded-lg text-sm flex items-center gap-3 muted opacity-60 cursor-not-allowed text-left">
            <span>👥</span> {{ 'settings.team-roles' | t }}
          </button>
        </div>

        <!-- Navigation Section: Businesses -->
        <div class="space-y-1">
          <p class="px-2 text-[10px] font-bold uppercase tracking-widest muted mb-2">{{ 'settings.businesses' | t }}</p>
      ...
      <div class="flex-1 p-6 md:p-8 overflow-y-auto" style="background: var(--tf-card);">
        ...
        <div class="flex items-center justify-between mb-8 pb-6 border-b" style="border-color: var(--tf-border);">
          <div>
            <h2 class="text-2xl font-bold">{{ tab === 'company' ? ('settings.company-profile' | t) : tab === 'security' ? ('settings.security' | t) : ('settings.title' | t) }}</h2>
          ...
          <div class="flex gap-3">
                <button (click)="loadData()" class="px-4 py-2 rounded-xl border text-sm hover:bg-[var(--tf-surface-2)] transition-all"
                  style="border-color: var(--tf-border);">{{ 'settings.cancel' | t }}</button>
            <button (click)="saveAll()" 
              ...
              {{ isSavingAll ? ('settings.saving' | t) : ('settings.save' | t) }}
            </button>
          </div>
        </div>
          ...
          <div *ngIf="tab === 'company'" class="space-y-8">
            ...
            <div class="tf-card !p-6">
              ...
              <div class="flex items-center gap-6 mb-8 p-4 rounded-xl border"
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs muted">{{ 'settings.upload-logo' | t }}</p>
                </div>
                ...
                <button type="button" (click)="fileInput.click()" class="px-4 py-2 rounded-lg border text-xs font-bold hover:bg-[var(--tf-surface-2)] transition-all"
                        style="border-color: var(--tf-border);">{{ 'settings.upload-logo' | t }}</button>
              </div>

              <form [formGroup]="companyForm" class="grid grid-cols-2 gap-6">
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.company-name' | t }}</label>
                ...
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.tax-id' | t }}</label>
                ...
                <div class="col-span-1 opacity-60">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.country' | t }}</label>
                ...
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.phone' | t }}</label>
                ...
                <div class="col-span-2">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.address' | t }}</label>
            ...
            <div class="tf-card !p-6">
              ...
              <form [formGroup]="businessForm" class="grid grid-cols-2 gap-6">
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.business-name' | t }}</label>
                  <input formControlName="name" class="w-full h-11 rounded-xl border px-4 text-sm outline-none transition-all focus:ring-2 focus:ring-primary-500/15 focus:border-primary-500"
                         style="border-color: var(--tf-border); background: var(--tf-surface); color: var(--tf-on-surface);"/>
                </div>
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.currency' | t }}</label>
                ...
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.tax-rate' | t }}</label>
                ...
                <div class="col-span-1">
                  <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.category' | t }}</label>
          ...
          <div *ngIf="tab === 'security'" class="space-y-6">
            <div class="tf-card !p-6">
              <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 class="text-lg font-bold">{{ 'settings.security-questions' | t }}</h3>
                  <p class="text-sm muted">{{ 'settings.security-questions-desc' | t }}</p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div *ngFor="let q of securityQuestions" class="relative group p-4 rounded-xl border transition-all duration-300 hover:bg-[var(--tf-surface-2)]"
                  ...
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <span class="text-xs font-bold uppercase tracking-wider text-primary-700 mb-1 block">{{ 'settings.new-question' | t }}</span>
              ...
              <div class="border-t pt-6" style="border-color: var(--tf-border);">
                <form [formGroup]="securityForm" (ngSubmit)="submitSecurityQuestion()" class="space-y-4 max-w-xl">
                  <div>
                    <label class="block text-[10px] font-bold uppercase tracking-widest muted mb-2">{{ 'settings.new-question' | t }}</label>
                  ...
                  <div>
                    <label class="block text-[10px] font-bold uppercase tracking-widest muted mb-2">{{ 'settings.answer' | t }}</label>
                  ...
                  <button type="submit" [disabled]="securityForm.invalid || securitySaving"
                    ...
                    {{ securitySaving ? ('settings.adding' | t) : ('settings.add-question' | t) }}
                  </button>
                </form>
              </div>
            </div>
            ...
            <div class="tf-card !p-6">
              <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 class="text-lg font-bold">{{ 'settings.twofa' | t }}</h3>
                  <p class="text-sm muted">{{ 'settings.twofa-desc' | t }}</p>
                </div>
              </div>
              ...
              <div class="p-4 rounded-xl border" style="border-color: var(--tf-border); background: var(--tf-surface);">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <p class="font-bold">{{ 'settings.status' | t }}: {{ user()?.is2faEnabled ? ('settings.enabled' | t) : ('settings.disabled' | t) }}</p>
                      <p class="text-xs muted">{{ user()?.is2faEnabled ? ('settings.enabled' | t) : ('settings.disabled' | t) }}</p>
                    </div>
                  </div>
                  ...
                  <button *ngIf="!user()?.is2faEnabled" 
                    ...
                    {{ isGenerating2fa ? ('settings.generating' | t) : ('settings.enable-2fa' | t) }}
                  </button>
    ...
    <div *ngIf="show2faModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] grid place-items-center p-6">
      <div class="w-full max-w-md border rounded-2xl p-8 shadow-2xl animate-in zoom-in duration-200 text-center"
        ...
        <h3 class="text-xl font-bold mb-2">{{ 'settings.set-up-2fa' | t }}</h3>
        <p class="text-sm muted mb-6">{{ 'settings.scan-qr' | t }}</p>
        ...
        <div class="space-y-4 text-left">
          <label class="block text-[10px] font-bold uppercase tracking-widest muted mb-2">{{ 'settings.enter-code' | t }}</label>
          ...
          <div class="flex gap-4 pt-4">
            ...
            <button (click)="onVerifyOtp()" 
              ...
              {{ isEnabling2fa ? ('settings.verifying' | t) : ('settings.verify-enable' | t) }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div *ngIf="showBusinessModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] grid place-items-center p-6">
      <div class="w-full max-w-lg border rounded-2xl p-8 shadow-2xl animate-in zoom-in duration-200"
           style="background: var(--tf-card); border-color: var(--tf-border); color: var(--tf-on-surface);">
        <h3 class="text-xl font-bold mb-2">{{ 'settings.create-business-title' | t }}</h3>
        <p class="text-sm muted mb-8">{{ 'settings.create-business-desc' | t:{ tenant: currentTenant()?.name || '' } }}</p>
        ...
        <form [formGroup]="newBusinessForm" (ngSubmit)="createBusiness()" class="space-y-6">
          ...
          <div class="flex gap-4 pt-4">
            ...
            <button type="submit" [disabled]="newBusinessForm.invalid || isCreatingBusiness" class="flex-1 h-11 rounded-xl bg-primary-600 hover:bg-primary-500 disabled:opacity-50 font-bold text-sm transition-all text-white">
              {{ isCreatingBusiness ? ('onboarding.creating' | t) : ('onboarding.create-go-dashboard' | t) }}
            </button>
          </div>
        </form>
      </div>
    ...
    <div *ngIf="showTenantModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] grid place-items-center p-6">
      <div class="w-full max-w-lg border rounded-2xl p-8 shadow-2xl animate-in zoom-in duration-200"
           style="background: var(--tf-card); border-color: var(--tf-border); color: var(--tf-on-surface);">
        <h3 class="text-xl font-bold mb-2">{{ 'settings.request-company-title' | t }}</h3>
        <p class="text-sm muted mb-8">{{ 'settings.request-company-desc' | t }}</p>
        ...
        <form [formGroup]="newTenantForm" (ngSubmit)="requestTenant()" class="space-y-6">
          <div class="grid grid-cols-2 gap-6">
            ...
            <div class="col-span-1">
              <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.tax-id' | t }}</label>
              <input formControlName="matricule" placeholder="1234567/A/M/000" class="w-full h-11 rounded-xl border px-4 text-sm outline-none transition-all focus:ring-2 focus:ring-primary-500/15 focus:border-primary-500"
                     style="border-color: var(--tf-border); background: var(--tf-surface); color: var(--tf-on-surface);"/>
            </div>
            <div class="col-span-1">
              <label class="block text-xs font-semibold muted uppercase mb-2 tracking-widest">{{ 'settings.phone' | t }}</label>
          ...
          <div class="flex gap-4 pt-4">
            ...
            <button type="submit" [disabled]="newTenantForm.invalid || isRequestingTenant" class="flex-1 h-11 rounded-xl bg-primary-600 hover:bg-primary-500 disabled:opacity-50 font-bold text-sm transition-all text-white">
              {{ isRequestingTenant ? ('settings.sending' | t) : ('settings.send-request' | t) }}
            </button>
          </div>
        </form>
      </div>
...
export class SettingsComponent implements OnInit, OnDestroy {
        ...
        if (this.activeBusinessId) {
          ...
          this.settings.updateBusiness(this.activeBusinessId, businessPayload).subscribe({
            next: () => {
              ...
              this.showToast(this.language.translate('settings.updated-success'), 'success');
              this.loadData();
            },
            error: (err) => {
              this.isSavingAll = false;
              this.showToast(err?.error?.message || this.language.translate('settings.request-error'), 'error');
            }
          });
        } else {
          this.isSavingAll = false;
          this.showToast(this.language.translate('settings.business-updated'), 'success');
          this.loadData();
        }
      },
      error: (err) => {
  ...
  createBusiness() {
    ...
    this.settings.createBusiness(this.newBusinessForm.value).subscribe({
      next: (res) => {
        ...
        this.showToast(this.language.translate('settings.created-success'), 'success');
      },
      error: () => {
        this.isCreatingBusiness = false;
        this.showToast(this.language.translate('settings.create-error'), 'error');
      }
    });
  }
  ...
  submitSecurityQuestion() {
    ...
    this.auth.setSecurityQuestions({ question: question!, answer: answer! }).subscribe(res => {
      ...
      this.showToast(this.language.translate('settings.security-added'), 'success');
    });
  }
