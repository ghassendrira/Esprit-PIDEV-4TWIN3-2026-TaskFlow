  ...
  template: `
    <div class="min-h-[calc(100vh-4rem)] p-6 bg-[var(--tf-surface)] text-[color:var(--tf-on-surface)]">
      ...
      <div class="rounded-2xl border border-[color:var(--tf-border)] p-5 shadow-card" style="background: var(--tf-card);">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="flex items-center gap-2 flex-wrap">
                <h1 class="text-2xl font-bold">{{ 'admin.blocked-title' | t }}</h1>
                <span class="inline-flex items-center rounded-full border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-2.5 py-1 text-xs text-[color:var(--tf-muted)]">
                  {{ 'admin.blocked-count' | t:{ count: blockedAccounts().length } }}
                </span>
              </div>
              <p class="text-sm text-[color:var(--tf-muted)] mt-1">{{ 'admin.blocked-description' | t }}</p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
          <div class="rounded-xl border border-[color:var(--tf-border)] p-4" style="background: var(--tf-surface-2);">
            <div class="text-xs text-[color:var(--tf-muted)]">{{ 'admin.blocked-users' | t }}</div>
            <div class="text-2xl font-bold mt-1">{{ blockedAccounts().length }}</div>
          </div>
          <div class="rounded-xl border border-[color:var(--tf-border)] p-4" style="background: var(--tf-surface-2);">
            <div class="text-xs text-[color:var(--tf-muted)]">{{ 'admin.unlocking-soon' | t }}</div>
            <div class="text-2xl font-bold mt-1 text-red-300">{{ unlockingSoonCount() }}</div>
          </div>
          <div class="rounded-xl border border-[color:var(--tf-border)] p-4" style="background: var(--tf-surface-2);">
            <div class="text-xs text-[color:var(--tf-muted)]">{{ 'admin.failed-attempts' | t }}</div>
      ...
      <div *ngIf="!loading() && blockedAccounts().length === 0" class="mt-6 rounded-2xl border border-[color:var(--tf-border)] p-10 text-center shadow-card" style="background: var(--tf-card);">
        ...
        <div class="text-xl font-semibold mt-4">{{ 'admin.no-blocked' | t }}</div>
        <div class="text-sm text-[color:var(--tf-muted)] mt-1">{{ 'admin.all-accessible' | t }}</div>
      </div>
              ...
              <div class="min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <span class="inline-flex items-center rounded-full border border-red-500/30 bg-red-950/20 px-2.5 py-1 text-xs text-red-200">
                    {{ remainingLabel(account) }} {{ 'admin.left' | t }}
                  </span>
                </div>
                ...
                <div class="mt-2 flex flex-wrap gap-2 text-xs text-[color:var(--tf-muted)]">
                  <span class="rounded-full border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-2.5 py-1">{{ 'admin.company' | t }}: {{ account.companyName || '—' }}</span>
                  <span class="rounded-full border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-2.5 py-1">{{ 'admin.role' | t }}: {{ account.roleName }}</span>
                  <span class="rounded-full border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-2.5 py-1">{{ 'admin.attempts' | t }}: {{ account.loginAttempts }}</span>
                </div>
                <div class="text-xs text-[color:var(--tf-muted)] mt-2">{{ 'admin.blocked-until' | t:{ date: ((account.blockedUntil | date:'medium') ?? '') } }}</div>
              </div>
            </div>
        ...
        <div class="w-[min(560px,96vw)] rounded-3xl border border-[color:var(--tf-border)] shadow-[0_30px_120px_rgba(0,0,0,.65)] overflow-hidden" style="background: var(--tf-card);">
          <div class="p-5 border-b border-[color:var(--tf-border)] flex items-start justify-between gap-3">
            <div class="flex items-start gap-3">
              ...
              <div>
                <div class="text-xl font-bold">{{ 'admin.unblock-title' | t }}</div>
          ...
          <div class="p-5 space-y-4">
            <div class="rounded-2xl border border-red-500/20 bg-red-950/20 px-4 py-3 text-sm text-red-100">
                  {{ 'admin.unblock-description' | t:{ date: ((pendingUnblockAccount()!.blockedUntil | date:'medium') ?? '') } }}
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div class="rounded-xl border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-4 py-3">
                <div class="text-xs text-[color:var(--tf-muted)] uppercase tracking-wider">{{ 'admin.unblock-company' | t }}</div>
                <div class="font-semibold mt-1">{{ pendingUnblockAccount()!.companyName || '—' }}</div>
              </div>
              <div class="rounded-xl border border-[color:var(--tf-border)] bg-[var(--tf-surface-2)] px-4 py-3">
                <div class="text-xs text-[color:var(--tf-muted)] uppercase tracking-wider">{{ 'admin.time-left' | t }}</div>
