import { Component, OnInit, inject, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Router } from '@angular/router';
import { AuthService } from '../../core/services/auth.service';
import { LanguageService } from '../../core/services/language.service';
import { TranslatePipe } from '../../shared/pipes/t.pipe';
import { AppDialogComponent } from '../../shared/components/app-dialog/app-dialog.component';

type PasswordResetRequest = {
  id: string;
  userId: string;
  requestedAt: string;
  user: {
    firstName: string;
    lastName: string;
    email: string;
  };
};

@Component({
  selector: 'tf-admin-password-requests',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslatePipe, AppDialogComponent],
  template: `
    <div class="min-h-[calc(100vh-4rem)] p-6 bg-[var(--tf-surface)] text-[color:var(--tf-on-surface)]">
      <!-- Toast -->
      <div *ngIf="toast()" class="fixed top-4 right-4 z-[60] w-[min(360px,92vw)]" role="status">
        <div
          class="rounded-xl border px-4 py-3 shadow-[0_12px_40px_rgba(0,0,0,.35)] backdrop-blur"
          style="background: var(--tf-card); border-color: var(--tf-border); color: var(--tf-on-surface);"
          [class.border-primary-500/40]="toast()!.type==='success'"
          [class.border-red-500/40]="toast()!.type==='error'"
          [class.bg-red-950/20]="toast()!.type==='error'"
        >
          <div class="flex items-start gap-3">
            <div class="mt-0.5">
              <svg
                *ngIf="toast()!.type==='success'"
                class="w-5 h-5 text-primary-600 dark:text-primary-200"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
              >
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 6L9 17l-5-5"/>
              </svg>
              <svg
                *ngIf="toast()!.type==='error'"
                class="w-5 h-5 text-red-300"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
              >
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </div>
            <div class="flex-1">
              <div class="font-semibold">{{ toast()!.title }}</div>
              <div class="text-sm text-[color:var(--tf-muted)] mt-0.5">{{ toast()!.message }}</div>
            </div>
          </div>
        </div>
      </div>

      <!-- Page header -->
      <div
        class="rounded-2xl border border-[color:var(--tf-border)] p-5 shadow-card"
        style="background: var(--tf-card);"
      >
        <div class="flex items-start gap-3">
          <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-primary-500/20 to-primary-700/20 border border-primary-500/20 grid place-items-center">
            <svg class="w-6 h-6 text-primary-700 dark:text-primary-200" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/>
            </svg>
          </div>
          <div>
            <h1 class="text-2xl font-bold">{{ 'admin.password-requests' | t }}</h1>
            <p class="text-sm text-[color:var(--tf-muted)] mt-1">{{ 'admin.password-requests-desc' | t }}</p>
          </div>
        </div>
      </div>

      <!-- Empty state -->
      <div
        *ngIf="!loading() && requests().length === 0"
        class="mt-6 rounded-2xl border border-[color:var(--tf-border)] p-10 text-center shadow-card"
        style="background: var(--tf-card);"
      >
        <div class="text-xl font-semibold">{{ 'admin.no-password-requests' | t }}</div>
        <div class="text-sm text-[color:var(--tf-muted)] mt-1">{{ 'admin.password-requests-empty' | t }}</div>
      </div>

      <!-- List -->
      <div *ngIf="requests().length > 0" class="mt-6 space-y-4">
        <div
          *ngFor="let r of requests()"
          class="rounded-2xl border border-[color:var(--tf-border)] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-card"
          style="background: var(--tf-card);"
        >
          <div class="flex items-start gap-4">
            <div class="w-12 h-12 rounded-xl bg-primary-500/10 border border-primary-500/20 grid place-items-center shrink-0">
              <span class="font-bold text-primary-700 dark:text-primary-200">{{ r.user.firstName[0] }}{{ r.user.lastName[0] }}</span>
            </div>
            <div>
              <div class="font-semibold text-lg">{{ r.user.firstName }} {{ r.user.lastName }}</div>
              <div class="text-sm text-[color:var(--tf-muted)]">{{ r.user.email }}</div>
              <div class="text-xs text-[color:var(--tf-muted)] mt-1">{{ 'admin.requested-at' | t:{ date: ((r.requestedAt | date:'medium') ?? '') } }}</div>
            </div>
          </div>

          <div class="flex items-center gap-2">
            <button
              (click)="approve(r)"
              class="px-4 py-2 rounded-xl bg-primary-600 hover:bg-primary-500 text-white text-sm font-semibold transition-colors flex items-center gap-2"
            >
              <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
              {{ 'admin.approve-send-password' | t }}
            </button>
            <button
              (click)="reject(r)"
              class="px-4 py-2 rounded-xl border border-red-500/50 text-red-200 hover:bg-red-950/30 text-sm font-semibold transition-colors"
            >
              {{ 'admin.reject-request' | t }}
            </button>
          </div>
        </div>
      </div>

      <tf-app-dialog
        [open]="approveDialogOpen()"
        title="Approve password reset"
        [message]="approveDialogMessage()"
        mode="confirm"
        confirmLabel="Approve"
        cancelLabel="Cancel"
        (confirm)="confirmApprove()"
        (cancel)="closeApproveDialog()"
      />

      <tf-app-dialog
        [open]="rejectDialogOpen()"
        title="Reject password reset"
        [message]="rejectDialogMessage()"
        mode="prompt"
        [value]="rejectReason()"
        inputLabel="Reason for rejection"
        placeholder="Write the rejection reason"
        confirmLabel="Reject"
        cancelLabel="Cancel"
        [danger]="true"
        (confirm)="confirmReject($event)"
        (cancel)="closeRejectDialog()"
      />
    </div>
  `,
})
export class AdminPasswordRequestsComponent implements OnInit {
  private auth = inject(AuthService);
  private router = inject(Router);
  private language = inject(LanguageService);

  requests = signal<PasswordResetRequest[]>([]);
  loading = signal(false);
  toast = signal<null | { type: 'success' | 'error'; title: string; message: string }>(null);
  approveDialogOpen = signal(false);
  approveDialogMessage = signal('');
  rejectDialogOpen = signal(false);
  rejectDialogMessage = signal('');
  rejectReason = signal('');
  selectedRequest = signal<PasswordResetRequest | null>(null);

  ngOnInit() {
    if (!this.auth.roles().includes('SUPER_ADMIN')) {
      this.router.navigate(['/dashboard']);
      return;
    }
    this.load();
  }

  load() {
    this.loading.set(true);
    this.auth.getPasswordResetRequests().subscribe({
      next: (data) => {
        this.requests.set(data);
        this.loading.set(false);
      },
      error: () => this.loading.set(false)
    });
  }

  approve(r: PasswordResetRequest) {
    this.selectedRequest.set(r);
    this.approveDialogMessage.set(
      this.language.translate('admin.confirm-password-reset', { email: r.user.email }),
    );
    this.approveDialogOpen.set(true);
  }

  confirmApprove() {
    const r = this.selectedRequest();
    if (!r) return;
    this.closeApproveDialog();
    this.auth.approvePasswordReset(r.id).subscribe({
      next: () => {
        this.toast.set({ type: 'success', title: this.language.translate('admin.approved'), message: this.language.translate('admin.password-sent') });
        this.load();
        setTimeout(() => this.toast.set(null), 3000);
      },
      error: () => this.toast.set({ type: 'error', title: 'Error', message: this.language.translate('admin.approve-failed') })
    });
  }

  reject(r: PasswordResetRequest) {
    this.selectedRequest.set(r);
    this.rejectReason.set('');
    this.rejectDialogMessage.set(`Reject password reset for ${r.user.email}?`);
    this.rejectDialogOpen.set(true);
  }

  confirmReject(reason: string) {
    const r = this.selectedRequest();
    if (!r) return;
    this.closeRejectDialog();
    this.auth.rejectPasswordReset(r.id, reason || 'Manual request rejected by admin.').subscribe({
      next: () => {
        this.toast.set({ type: 'success', title: this.language.translate('admin.rejected'), message: this.language.translate('admin.user-notified') });
        this.load();
        setTimeout(() => this.toast.set(null), 3000);
      },
      error: () => this.toast.set({ type: 'error', title: 'Error', message: this.language.translate('admin.reject-failed') })
    });
  }

  closeApproveDialog() {
    this.approveDialogOpen.set(false);
    this.selectedRequest.set(null);
  }

  closeRejectDialog() {
    this.rejectDialogOpen.set(false);
    this.rejectReason.set('');
    this.selectedRequest.set(null);
  }
}
