  ...
  template: `
    <div class="min-h-[calc(100vh-3rem)] p-6 bg-[var(--tf-surface)] text-[var(--tf-on-surface)]">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">{{ 'roles.title' | t }}</h1>
          <p class="text-sm text-[var(--tf-muted)] mt-1">{{ 'roles.subtitle' | t }}</p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="stat-card">
          <span class="stat-label">{{ 'roles.total' | t }}</span>
          <div class="stat-value">{{ roles().length }}</div>
        </div>
        <div class="stat-card">
          <span class="stat-label">{{ 'roles.permissions' | t }}</span>
          <div class="stat-value">{{ permissions().length }}</div>
        </div>
        <div class="stat-card">
          <span class="stat-label">{{ 'roles.active-users' | t }}</span>
          <div class="stat-value">{{ activeUsers() }}</div>
        </div>
        <div class="stat-card">
          <span class="stat-label">{{ 'roles.full-access' | t }}</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
        <div *ngFor="let role of roles()" class="role-card" [style.border-top-color]="getRoleColor(role.name)">
          ...
          <div class="mt-auto pt-4 border-t border-[var(--tf-border)] flex items-center gap-2 text-xs text-[var(--tf-muted)]">
            ...
            {{ role.userCount || 0 }} {{ 'roles.users-active' | t }}
          </div>
        </div>
      </div>
      ...
      <div class="rounded-2xl border border-[var(--tf-border)] bg-[var(--tf-card)] overflow-hidden shadow-[var(--tf-shadow)]">
        <div class="p-6 border-b border-[var(--tf-border)]">
          <h2 class="text-lg font-bold">{{ 'roles.matrix' | t }}</h2>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-[var(--tf-surface-2)]">
                <th class="p-4 text-xs font-bold uppercase tracking-widest text-[var(--tf-muted)] border-b border-[var(--tf-border)]">{{ 'roles.permission' | t }}</th>
...
export class RolesPermissionsComponent implements OnInit {
  ...
  getRoleDescription(roleName: string): string {
    switch (roleName) {
      case 'SUPER_ADMIN': return this.translate('roles.super-admin-desc');
      case 'ADMIN': return this.translate('roles.admin-desc');
      case 'OWNER': return this.translate('roles.owner-desc');
      case 'BUSINESS_OWNER': return this.translate('roles.business-owner-desc');
      default: return this.translate('roles.custom-desc');
    }
  }
