import { Component, inject, ChangeDetectorRef, OnDestroy, ViewChild, AfterViewInit, NgZone } from '@angular/core';
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, RouterLink, NgIf, TranslatePipe, RecaptchaComponent],
  template: `
    <div class="min-h-screen grid place-items-center p-6 bg-gradient-to-b from-[var(--tf-surface)] to-[var(--tf-surface-2)]">
      <div class="relative w-[min(520px,92vw)] tf-card rounded-2xl p-6 sm:p-8 text-[var(--tf-on-surface)]">
        ...
        <button
          ...
          [attr.aria-label]="'register.back-to-login' | t"
        >
          <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
          {{ 'register.back-to-login' | t }}
        </button>
        <div class="flex flex-col items-center text-center mb-6 mt-1">
          ...
          <h2 class="mt-4 text-2xl font-bold">{{ 'common.create-account' | t }}</h2>
          <p class="mt-1 text-sm text-[var(--tf-muted)]">{{ 'auth.create-account-desc' | t }}</p>
        </div>
        ...
        <div *ngIf="showSuccessBanner" class="mb-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-5 py-4 text-center shadow-sm" aria-live="polite">
          <div class="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400 mb-1">{{ 'register.sent' | t }}</div>
        ...
        <form class="space-y-4" [formGroup]="form" (ngSubmit)="onSubmit()">
          ...
          <div>
            <label class="block text-[10px] font-semibold uppercase tracking-widest text-[var(--tf-muted)] mb-2">{{ 'common.company-name' | t }}</label>
            <div class="relative">
          ...
          <div>
            <label class="block text-[10px] font-semibold uppercase tracking-widest text-[var(--tf-muted)] mb-2">{{ 'auth.company-category' | t }}</label>
            <select formControlName="companyCategory"
              ...
              <option value="">{{ 'auth.company-category-select' | t }}</option>
              <option value="Retail">{{ 'auth.category-retail' | t }}</option>
              <option value="Services">{{ 'auth.category-services' | t }}</option>
              <option value="Technology">{{ 'auth.category-technology' | t }}</option>
              <option value="Restaurant">{{ 'auth.category-restaurant' | t }}</option>
              <option value="Freelance">{{ 'auth.category-freelance' | t }}</option>
              <option value="Healthcare">{{ 'auth.category-healthcare' | t }}</option>
              <option value="Education">{{ 'auth.category-education' | t }}</option>
              <option value="Other">{{ 'auth.category-other' | t }}</option>
            </select>
          </div>
          <div>
            <label class="block text-[10px] font-semibold uppercase tracking-widest text-[var(--tf-muted)] mb-2">{{ 'auth.professional-email' | t }}</label>
            <div class="relative">
              <span class="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[var(--tf-muted)]">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              <input type="email" formControlName="email" required maxlength="254" autocomplete="email" [placeholder]="'auth.email-placeholder' | t"
                     class="w-full h-12 rounded-xl bg-[var(--tf-surface)] border border-[var(--tf-border)] pl-12 pr-4 text-[var(--tf-on-surface)] placeholder:text-[var(--tf-muted)] outline-none focus:ring-2 focus:ring-[var(--tf-primary)] focus:border-transparent transition"/>
            </div>
            <p *ngIf="showError('email', 'required')" class="mt-1 text-xs text-red-400">{{ 'auth.email-required' | t }}</p>
            <p *ngIf="showError('email', 'email')" class="mt-1 text-xs text-red-400">{{ 'auth.invalid-email' | t }}</p>
          ...
          <p *ngIf="showError('accept', 'requiredTrue')" class="mt-1 text-xs text-red-400">{{ 'auth.accept-required' | t }}</p>
          ...
          <button class="w-full h-12 rounded-xl px-4 bg-[var(--tf-primary)] text-white dark:text-slate-900 font-semibold hover:brightness-95 transition disabled:opacity-50"
              [disabled]="form.invalid || isSubmitting || !recaptchaToken">
            {{ 'common.register' | t }}
          </button>
          <div class="flex items-center gap-3 text-[var(--tf-muted)] text-xs">
            <div class="flex-1 h-px bg-[var(--tf-border)]"></div>
            {{ 'register.continue-with' | t }}
            <div class="flex-1 h-px bg-[var(--tf-border)]"></div>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button type="button" (click)="onGoogleSignup()" [disabled]="isGoogleLoading" class="relative w-full h-12 rounded-[10px] bg-[var(--tf-surface-2)] text-[var(--tf-on-surface)] border border-[var(--tf-border)] hover:brightness-95 transition-colors duration-200 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              <span class="block text-center">{{ isGoogleLoading ? ('common.please-wait' | t) : ('auth.social-google' | t) }}</span>
            </button>
            <button type="button" class="relative w-full h-12 rounded-[10px] bg-[var(--tf-surface-2)] text-[var(--tf-on-surface)] border border-[var(--tf-border)] hover:brightness-95 transition-colors duration-200 transform hover:-translate-y-0.5">
              ...
              <span class="block text-center">{{ 'auth.social-facebook' | t }}</span>
            </button>
            <button type="button" class="relative w-full h-12 rounded-[10px] bg-[var(--tf-surface-2)] text-[var(--tf-on-surface)] border border-[var(--tf-border)] hover:brightness-95 transition-colors duration-200 transform hover:-translate-y-0.5">
              <span class="absolute left-3 top-1/2 -translate-y-1/2">
                <svg width="18" height="18" viewBox="0 0 24 24">
              ...
              <span class="block text-center">{{ 'auth.social-linkedin' | t }}</span>
            </button>
            <button type="button" class="relative w-full h-12 rounded-[10px] bg-[var(--tf-surface-2)] text-[var(--tf-on-surface)] border border-[var(--tf-border)] hover:brightness-95 transition-colors duration-200 transform hover:-translate-y-0.5">
              <span class="absolute left-3 top-1/2 -translate-y-1/2">
                <svg width="18" height="18" viewBox="0 0 24 24">
              ...
              <span class="block text-center">{{ 'auth.social-apple' | t }}</span>
            </button>
          </div>
        </form>
        ...
        <div class="text-sm text-[var(--tf-muted)] mt-6 text-center">
          {{ 'auth.already-account' | t }}
          <a class="text-[var(--tf-primary)] hover:underline" routerLink="/auth/login">{{ 'common.login' | t }}</a>
        </div>
...
export class RegisterComponent implements AfterViewInit, OnDestroy {
  ...
  private handleGoogleCodeResponse(response: any) {
    ...
    const companyName = this.form.value.company?.trim() || undefined;
    const companyCategory = this.form.value.companyCategory?.trim() || undefined;
    ...
    this.auth.googleSignup({
      ...
      next: (res: any) => {
        ...
        // If a token is returned, it means it's a login for an existing active user
        if (res?.data?.token) {
        ...
        this.successMessage = res?.message || this.language.translate('auth.register-success-message');
        this.showSuccessBanner = true;
        this.form.reset({
          firstName: '', lastName: '', company: '', companyCategory: '', email: '', accept: false,
        });
      ...
      error: (err: any) => {
        ...
        this.errorMessage = typeof msg === 'string' ? msg : this.language.translate('common.unknown-error');
        this.cdr.detectChanges();
      },
    });
  }
  ...
  onSubmit() {
    ...
    this.auth
      ...
      next: (response: any) => {
        ...
        this.submitted = false;
        this.fieldErrors = {};
        this.successMessage = response?.message || this.language.translate('auth.register-success-message');
        this.showSuccessBanner = true;
        this.recaptchaToken = '';
        this.captchaRef?.reset();
        this.successTimer = window.setTimeout(() => {
      ...
      error: (error) => {
        if (error?.name === 'TimeoutError') {
          this.errorMessage = this.language.translate('common.please-wait');
        } else if (error?.status === 409) {
          this.errorMessage = this.language.translate('auth.email-already-used');
        } else if (error?.status === 400) {
          this.applyBackendValidationErrors(error?.error?.message);
          if (!Object.keys(this.fieldErrors).length) {
            this.errorMessage = typeof error?.error?.message === 'string' ? error.error.message : this.language.translate('common.unknown-error');
          }
        } else {
  ...
  private applyBackendValidationErrors(message: string | string[] | undefined) {
    ...
    for (const msg of messages) {
      ...
      if (lower.includes('firstname') || lower.includes('first name')) {
        ...
        this.fieldErrors.email = this.language.translate('auth.invalid-email');
      } else if (lower.includes('accept')) {
        this.fieldErrors.accept = this.language.translate('auth.accept-required');
      }
    }
  }
