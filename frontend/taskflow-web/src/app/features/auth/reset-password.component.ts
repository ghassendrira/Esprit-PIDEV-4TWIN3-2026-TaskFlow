      ...
      <div class="relative w-[min(420px,92vw)] tf-card rounded-2xl p-6 text-[var(--tf-on-surface)]">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
          ...
          <div class="text-xs text-[var(--tf-muted)]">
            {{ 'forgot.min-eight' | t }}, {{ 'forgot.uppercase' | t }}, {{ 'forgot.lowercase' | t }}, {{ 'forgot.number' | t }}
          </div>
          ...
          <button class="w-full rounded-lg px-4 py-2 bg-[var(--tf-primary)] text-white dark:text-slate-900 font-semibold disabled:opacity-50" [disabled]="form.invalid || loading">{{ 'reset.update' | t }}</button>
        </form>
      </div>
    </div>
  `,
...
export class ResetPasswordComponent {
  ...
  submit() {
    ...
    if (!token) {
      this.errorMessage = this.language.translate('reset.missing-token');
      return;
    }
    this.loading = true;
    ...
    this.auth
      ...
      .subscribe({
        next: () => {
          this.loading = false;
          this.successMessage = this.language.translate('reset.success');
          setTimeout(() => this.router.navigateByUrl('/auth/login'), 1200);
        },
        error: (err) => {
          this.loading = false;
          this.errorMessage = err?.error?.message ?? err?.message ?? this.language.translate('common.unknown-error');
        },
      });
  }
}
