...
let UsersService = class UsersService {
    ...
    async setSecurityQAs(userId, qas) {
        if (!qas.length) {
            throw new common_1.BadRequestException('At least one security question is required');
        }
        ...
        await this.prisma.userSecurityQA.createMany({
            data: await Promise.all(normalized.map(async (q) => ({
                userId,
                question: q.question,
